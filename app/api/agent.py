...
### [2. 필터(Filters) 활용 (중요!)]
사용자가 구체적인 제약 조건을 말했을 때만 `filters`에 값을 넣으세요.
**[⚠️ 매우 중요 - NULL 금지]**
- 사용자가 조건을 언급하지 않았다면, **해당 키(Key) 자체를 절대 보내지 마세요 (Omit the key).**
- **절대로** `null` 값이나 빈 문자열(`""`)을 `filters` 값으로 채우지 마세요.
- **나쁜 예:** `{"max_price": null, "category_name": ""}` (❌ 에러 발생!)
- **좋은 예:** `{"min_rating": 9.0}` (⭕ 나머지는 아예 생략)

### [3. 도구 해석 가이드 (Tool Interpretation Guide)]
